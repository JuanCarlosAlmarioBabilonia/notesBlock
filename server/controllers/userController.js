...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: usuarioGuardado._id, rol: usuarioGuardado.rol }, SECRET_KEY, { expiresIn: 18000000 });

    req.session.auth = { id: usuarioGuardado._id, token }; 
    res.status(201).json({ message: "Usuario creado", data: usuarioGuardado, token });
